...
- **Task 10, the live check.** Watching the real app from the local Prometheus showed `OrdersSilent` firing a few minutes after the app started. `min_over_time` sees only the samples a series has, so a three-minute-old `business_open` passed "open for the whole window", and every deploy during opening hours would have notified. The rule now also requires `count_over_time(...) >= 15`, and a fresh-deploy case was added that fails without it.
- **Task 10, dependencies.** The registry was unreachable (DNS) when `yaml` was due to be added, so the cross-check reads the rules with `js-yaml@4.3.0` from the local store, with a one-function type declaration in `test/fixtures/js-yaml.d.ts`. Grafana's image could not be pulled either, so its provisioning is checked by the cross-check test only, not by a live render. The same outage surfaced that `prom-client` is deprecated in favour of `@prometheus-io/client`; see the slice's closing notes.
- **Closing: the client library.** Once the registry was reachable again, `@prometheus-io/client` checked out as `prom-client`'s continuation under the Prometheus organisation, with the same `reset()`, `get()`, content type and default metric names this slice depends on. The slice moved to it. Its typings resolve an unlabelled metric's labels to `never` (a conditional type distributing over `never`), so `ScrapedGauge` keeps its own `Sample` type and casts once in `get()`.
