...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    // route with neither decorator has no contract, so it fails closed and
    // says why — rather than defaulting to "any authenticated principal",
    // which is how a forgotten decorator on an admin endpoint reaches a kiosk.
